        ...
        for f in checkpoint_folders:
            for item in os.listdir(f):
                if item.endswith(".safetensors"):
                    self.checkpoints.append(os.path.join(f, item))

        # Apply the real_length limit *after* filtering
        if real_length is not None:
            self.checkpoints = self.checkpoints[: min(len(self.checkpoints), real_length)]
